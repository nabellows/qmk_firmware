...
association, physical x/y position, and flags.
...
def infer_output_path(keyboard_target: str) -> Path:
    flattened = keyboard_target.replace("/", "_")
    return Path.cwd() / f"{flattened}.js"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        context = build_context(args, info, keyboard_target)

        output = args.output.expanduser() if args.output else infer_output_path(keyboard_target)
